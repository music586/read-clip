...
| 路径 | 功能 |
| --- | --- |
| `/` | 阅读索引、文章数量、标签数量、最近收录时间和最新文章列表 |
| `/clips/{id}/` | 文章详情、标签导航、复制正文和复制链接 |
| `/tags/` | 标签总览和每个标签的文章数量 |
...
文章详情页提供“首页 → 分类 → 当前标签”的面包屑导航。面包屑、标签和复制按钮都位于正文语义区域之外，并排除 Pagefind 索引；浏览器阅读模式和稍后读工具会优先提取纯 Markdown 正文，不会把这些界面元素拼进文章内容。
